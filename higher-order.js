...
let evenNumbers = mixedNumbers.filter( y => y%2 ===0)
...
let totalPopulation  = populations.reduce(function(prev, pop) {
    return prev+=pop;
})
...
var myStrongest  = monstersInYourPocket.filter(y => y.CP > 200)
...
let ordersTotal  = orders.map(y => y.price * (1+y.tax));
